...
def prepare_runtime_repository(
    ...
    runtime_root = workspace_dir / "runtime_repo"
    ...
    shutil.copytree(
        repo_root / "manual_gold",
        runtime_root / "manual_gold",
    )
    ...
    for filename in (
        ...
        source = repo_root / "data" / filename
        if source.is_file():
            shutil.copy2(
                source,
                runtime_root / "data" / filename,
            )
    ...
    runtime_review.mkdir(parents=True)
    shutil.copy2(
        repo_root
        / "review"
...
def export_flat_replay_results(
    repo_root: Path,
    replay_log: dict[str, Any],
    cross_check: dict[str, Any],
    gold_merge: dict[str, Any],
) -> None:
    write_json(
        repo_root
        / "logs"
    ...
    write_json(
        repo_root
        / "validation"
        / "reproducibility_test.json",
        cross_check,
    )
    review_summary_path = (
        repo_root / "review" / "review_summary.json"
    )
    review_summary = (
        read_json(review_summary_path)
        if review_summary_path.is_file()
        else {}
    )
    review_summary["replay_gold_merge"] = gold_merge
...
def run_all(
    ...
    started_at = now_iso()
    runtime_root = prepare_runtime_repository(
        repo_root,
        workspace_dir,
    )
    runtime_workspace = workspace_dir / "runtime_workspace"
    ...
    replay_log = {
        ...
        "runtime_repository": "workspace/runtime_repo",
        "run_ids": run_ids,
        "stage_status": stage_status,
        "gold_merge": gold_merge,
        "cross_check": cross_check,
        "submission_result_directories_remain_flat": True,
    }
    export_flat_replay_results(
        repo_root,
        replay_log,
        cross_check,
        gold_merge,
    ...
    print("人工Gold开放复核：0")
    print("validation、review、logs提交目录保持扁平")
